...
export function SelecionaArea({ onComplete, isLoading = false, multiple = true, showHeader = true }: SelecionaAreaProps) {
          ...
          <Card className="w-full max-w-2xl shadow-lg overflow-hidden">
            <CardHeader className="bg-gradient-to-r from-[#03565C] to-[#24BFB0] text-white">
              ...
              <CardDescription className="text-white/90">
                Selecione {multiple ? "uma ou mais áreas" : "a área"} onde você tem maior experiência
              </CardDescription>
            </CardHeader>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className={`font-semibold transition-colors ${
                            selectedAreas.includes(area.id) ? "text-[#03565C]" : "text-gray-900"
                          }`}>
                            {area.nome}
                          </h3>
                ...
                <Alert className="border-[#24BFB0]/30 bg-[#25D9B8]/10">
                  ...
                  <AlertDescription className="text-[#03565C] text-sm">
                    Você poderá ajustar suas áreas de atuação depois, durante a autoavaliação de competências.
                  </AlertDescription>
                </Alert>
                ...
                >
                  {submitting ? "Salvando..." : isLoading ? "Carregando..." : `Continuar com ${selectedAreas.length} ${selectedAreas.length === 1 ? "área" : "áreas"} selecionada${selectedAreas.length === 1 ? "" : "s"}`}
